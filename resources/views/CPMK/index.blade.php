...
@section('content')
    ...
    <div class="container">
        ...
        <div class="container-title mb-4">
            <h1 class="display-5">Daftar CPMK</h1>
            <p class="lead text-muted">Lihat informasi kode CPMK dan deskripsi di bawah ini.</p>
        </div>
        ...
        <div class="card p-4 shadow-lg border-0 mb-5 border-top"
            ...
            <div class="d-flex justify-content-between mb-3">
                <!-- Tombol Tambah -->
                <a href="{{ url('/CPMK/create') }}" type="button" class="btn btn-success custom-btn-green">
                ...
                <table class="table table-bordered table-hover table-striped">
                    <thead class="custom-thead">
                        <tr>
                            ...
                            <th scope="col" style="color: #ffffff !important; background-color: #0052A2 !important;">CPMK
                            </th>
                            <th scope="col"
                    ...
                    <tbody>
                        @foreach ($cpmk as $index => $cpmkes)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $cpmkes->deskripsi }}</td>
                                <td>{{ $cpmkes->kode_cpmk }}</td>
                                <td>
                                    <!-- Tombol Edit -->
                                    <a href="{{ route('cpmk.edit', $cpmkes->id) }}" class="btn btn-warning btn-sm">
                                        <i class="bi bi-pencil"></i> Ubah
                                    </a>

                                    <!-- Form Hapus -->
                                    <form action="{{ route('cpmk.destroy', $cpmkes->id) }}" method="POST" class="d-inline"
