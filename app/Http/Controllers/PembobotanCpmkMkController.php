    ...
    try {
        ...
        if ($user->role === 'dosen') {
            ...
            $mks = Mk::where('kode_prodi', $kodeProdi)
                ...
                ->get();
            Log::info('Loaded MKs for KPS, count: ' . $mks->count() . ', kode_prodi: ' . $kodeProdi . ', kurikulumId: ' . $kurikulumId);
        }
        ...
        if ($defaultMk) {
            ...
            Log::info('Initial CPMKs loaded for mk_id: ' . ($defaultMk->id ?? 'null') . ', count: ' . count($cpmks));
        }
    ...
    public function searchMk(Request $request)
{
    ...
    try {
        ...
        // Inisialisasi $mkIds berdasarkan peran
        $mkIds = collect();
        if ($user->role === 'dosen') {
            $nip = $user->nip;
            if (!$nip) {
            ...
            Log::info('MK IDs from Kelas for NIP ' . $nip . ': ' . json_encode($mkIds->toArray()));
        ...
        // Ambil mkIds yang memiliki cpmks untuk semua peran
        $mksWithCpmks = Mk::where('kode_prodi', $kodeProdi)
            ->whereHas('cpmks', function ($query) use ($kurikulumId) {
                $query->where('kurikulum_id', $kurikulumId);
            })
            ->when($user->role === 'dosen', function ($query) use ($mkIds) {
                $query->whereIn('id', $mkIds);
            })
            ->pluck('id')
            ->toArray();
        Log::info('MK IDs with CPMKs for kurikulumId ' . $kurikulumId . ': ' . json_encode($mksWithCpmks));

        // Terapkan filter berdasarkan peran
        if ($user->role === 'dosen') {
            $mksQuery->whereIn('id', $mkIds);
        } else {
            $mksQuery->whereHas('cpmks', function ($query) use ($kurikulumId) {
                $query->where('kurikulum_id', $kurikulumId);
            });
        }
        ...
        $mks = $mksQuery->get()->map(function ($mk) use ($mksWithCpmks) {
            $text = "{$mk->kode_mk} - {$mk->deskripsi}";
            if (!in_array($mk->id, $mksWithCpmks)) {
                $text .= ' (Tidak ada data pemetaan CPMK-MK)';
            }
            return [
                'id' => $mk->id,
                'text' => $text,
        ...
        Log::info('Search MK results for kode_prodi: ' . $kodeProdi . ', tahunFilter: ' . $tahunFilter . ', kurikulumId: ' . $kurikulumId . ', count: ' . $mks->count() . ', mkIds: ' . json_encode($mks->pluck('id')->toArray()) . ', mksWithCpmks: ' . json_encode($mksWithCpmks));
...
{
    $user = Auth::user();
    $kodeProdi = $user->kode_prodi; // Pastikan $kodeProdi didefinisikan dari user
    ...
    try {
        ...
        Log::info("Fetching CPMKs for mk_id: {$mk_id}, kode_prodi: {$kodeProdi}, tahunFilter: {$tahunFilter}, kurikulumId: {$kurikulumId}");

        // Ambil mata kuliah berdasarkan mk_id
        $mk = Mk::where('id', $mk_id)
                ->where('kode_prodi', $kodeProdi)
                ->firstOrFail();

        // Ambil CPMK langsung dari tabel cpmk_mk untuk memastikan data sesuai
        $cpmks = DB::table('cpmk_mk')
            ->join('cpmk', 'cpmk_mk.cpmk_id', '=', 'cpmk.id')
            ->where('cpmk_mk.mk_id', $mk_id)
            ...
            ->get()
            ->map(function ($cpmk) use ($mk_id, $kurikulumId, $kodeProdi) {
                $teknikPenilaianRaw = DB::table('teknik_penilaian')
                    ->where('mk_id', $mk_id)
                    ->where('cpmk_id', $cpmk->id)
                    ->where('kurikulum_id', $kurikulumId)
                ...
                Log::info('CPMK Data (getCpmks) for mk_id: ' . $mk_id . ', kode_prodi: ' . $kodeProdi . ', kurikulumId: ' . $kurikulumId . ': ' . json_encode([
                    'id' => $cpmk->id,
                    'kode_cpmk' => $cpmk->kode_cpmk,
                    'deskripsi' => $cpmk->deskripsi ?? 'Deskripsi tidak tersedia',
                    'bobot' => $cpmk->bobot ?? 0,
    ...
    public function update(Request $request)
{
    ...
    try {
        ...
        // Hanya filter berdasarkan kode_prodi untuk KPS, hapus whereHas('kelas') jika peran KPS
