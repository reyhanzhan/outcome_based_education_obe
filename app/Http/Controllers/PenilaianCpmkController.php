    ...
    public function index($mahasiswa_id, $mk_id)
{
    ...
    $cpmks = Cpmk::whereHas('mks', function ($query) use ($mk_id) {
        ...
        'mks' => function ($query) use ($mk_id) {
            $query->where('mk_id', $mk_id)->withPivot('bobot', 'min_standard', 'jumlah_penilaian');
        },
    ...
    // Ambil jumlah penilaian dari tabel cpmk_mk
    $jumlahPenilaian = \Illuminate\Support\Facades\DB::table('cpmk_mk')
        ->where('mk_id', $mk_id)
        ->value('jumlah_penilaian') ?? 3; // Default ke 3 jika tidak ada data
    Log::info('Jumlah Penilaian from cpmk_mk:', ['jumlahPenilaian' => $jumlahPenilaian]);
